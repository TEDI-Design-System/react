// forked from design-sync lib/dts.mjs — this package declares its .d.ts entry ONLY
// under exports["./tedi"].types, with no top-level `types`/`typings`, so upstream's
// entry resolution fell through to the nonexistent <dist>/index.d.ts and discovered
// zero exports (every storybook title then dropped as [TITLE_UNMAPPED]).
//
// The fork adds ONE thing: subpathTypes(), an exports-map lookup consulted by both
// entry-resolution sites (findTypesRoot and projectFor) before their existing
// fallbacks. Everything else is upstream verbatim — diff against
// .ds-sync/lib/dts.mjs on re-sync and merge upstream changes.
//
// The subpath is pinned to './tedi' on purpose: this sync covers the TEDI-Ready
// namespace only, never './community' (see .design-sync/NOTES.md § Scope). That
// makes the tedi-only scope structural rather than a flag someone can forget.

import { existsSync, readFileSync, readdirSync } from 'node:fs';
import { dirname, join } from 'node:path';
import { Project, Node, ts } from 'ts-morph';

// The exports subpath this sync draws its component surface from.
const TYPES_SUBPATH = './tedi';

// Resolve the .d.ts entry from the package's exports map. Returns an absolute
// path that exists on disk, or null. Only the conditions TypeScript itself would
// consult are read ("types", then "import"/"default" objects nested under them).
function subpathTypes(pkgDir, pkgJson) {
  const ent = pkgJson?.exports?.[TYPES_SUBPATH];
  if (!ent) return null;
  const seen = new Set();
  const walk = (node) => {
...
export function findTypesRoot(pkgDir, pkgJson) {
  // FORK: exports-map types entry wins — this package has no top-level `types`.
  const sub = subpathTypes(pkgDir, pkgJson);
  if (sub) return dirname(sub);
...
function projectFor(pkgDir, typesRoot) {
  ...
  // FORK: exports-map types entry wins here too, or `entry` would still point at
  // the nonexistent <dist>/index.d.ts and getSourceFile(entry) would return
  // undefined — findTypesRoot alone is not enough, this site resolves `entry`
  // independently of typesRoot.
  const pubEntry = pj.publishConfig?.types;
  const entry = subpathTypes(pkgDir, pj)
    || join(pkgDir, (pubEntry && existsSync(join(pkgDir, pubEntry)) ? pubEntry : null) || pj.types || pj.typings || 'index.d.ts');
