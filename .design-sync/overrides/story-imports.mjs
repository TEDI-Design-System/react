...
const MANAGER_API_STUB =
  ...
  // FORK: useGlobals() returns the globalTypes DEFAULTS, statefully.
  // theme-provider.stories.tsx imports useGlobals from
  // 'storybook/internal/preview-api' and prints `Current theme: {globals.theme}`.
  // Upstream's stub returns an empty object, so the readout rendered BLANK while
  // storybook showed "default". Seeding ctx.globals in preview-gen could not fix
  // this: the story reads the addon hook, not the render context.
  // Stateful (React.useState) rather than a frozen literal, because that is what
  // storybook does — so the card's own Default/Dark toggle updates the readout
  // instead of looking inert. Mirrors globalTypes in .storybook/preview.tsx; keep
  // in sync with GLOBAL_DEFAULTS in overrides/preview-gen-storybook.mjs.
  'const GD={theme:"default"};' +
  'module.exports={addons,types:{},' +
  'useGlobals(){const s=R().useState(GD);return[s[0],function(n){s[1](Object.assign({},s[0],n))}]},' +
  'useArgs(){return[{},function(){},function(){}]},useParameter(){},useStorybookApi(){return{}},' +
...
// renders.
// FORK: kebab-case filenames -> PascalCase export names.
//
// Upstream compares the bare filename against the export set case-sensitively
// (`exported.has('button')` for button.tsx), which assumes the DS names files
// after its exports (Button.tsx). This repo names every file in kebab-case
// (button.tsx, vertical-spacing.tsx, button-content.tsx) while exporting
// PascalCase, so NOTHING ever matched and NOT ONE component was shimmed to the
// shipped bundle. Every preview instead compiled the component from src/, where
// esbuild loads `.module.scss` as EMPTY (no sass) — so every CSS-module class
// lookup returned undefined and all 81 previews rendered completely unstyled
// while still "rendering cleanly" for the validator. Only plain global classes
// (Print's `no-print`) survived, which is what made the breakage look subtle.
//
// pascal() is additive: the upstream exact-match checks are tried FIRST and are
// unchanged, so a DS that does name files after exports behaves identically.
// Over-shimming is self-announcing (undefined-component cell errors), and
// candidates are still gated on membership in `exported`.
// Separator-and-case-insensitive lookup: strip [-_] and lowercase both sides, so
// ANY filename spelling reaches its export. This replaced a naive pascal() that
// only split on [-_] and therefore still missed every camel-humped export whose
// filename is a single lowercase word or hyphenated differently:
//   textfield        -> pascal 'Textfield'        != TextField
//   textarea         -> pascal 'Textarea'         != TextArea
//   sidenav          -> pascal 'Sidenav'          != SideNav
//   sidenav-item     -> pascal 'SidenavItem'      != SideNavItem
//   top-nav-subitem  -> pascal 'TopNavSubitem'    != TopNavSubItem
//   use-labels       -> pascal 'UseLabels'        != useLabels
// Those files kept compiling from src/ with an EMPTY .module.scss, so TextField's
// icon fell out of the input, size="small" was a no-op, --valid/--invalid borders
// were dead, and SideNav.Item rendered with no styling at all.
// Cached per export-set identity — this runs on every resolved import.
const ciCache = new WeakMap();
const ciLookup = (exported) => {
  let m = ciCache.get(exported);
  if (!m) {
    m = new Map();
    for (const n of exported) m.set(n.replace(/[-_]/g, '').toLowerCase(), n);
    ciCache.set(exported, m);
  }
  return m;
};
...
function exportedComponentFor(p, exported) {
  ...
  // FORK: same two checks again, through the kebab -> Pascal conversion — but ONLY
  // for files under src/tedi/. The kebab->Pascal widening makes accidental matches
  // possible in a way the exact-match checks never were, and both misfires were real:
  //   - src/community/components/tabs/… pascal-matched TEDI's exported `Tabs`, so
  //     hash-trigger.stories (which imports community Tabs + TabsItem) got the WRONG
  //     component and crashed with "Element type is invalid" on the absent TabsItem.
  //     Community is deliberately outside this sync (see NOTES.md § Scope) and must
  //     never resolve to the tedi bundle.
  // The remaining in-subtree hazard (generic names like `Input`/`Prefix`/`Suffix`
  // shimming out of a src-compiled parent and splitting its React context) is not a
  // path problem and is handled per-subtree by cfg.storyImports.bundle instead.
  if (/\/src\/tedi\//.test(p.replace(/\\/g, '/'))) {
    const ci = ciLookup(exported);
    const hit = ci.get(squash(file));
    if (hit) return hit;
    if (file === 'index' || file === dir) {
      const dirHit = ci.get(squash(dir));
