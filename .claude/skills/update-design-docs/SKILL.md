...
Keep `DESIGN.md` and `design-tokens/component.manifest.json` current. **Generators own the data;
...
2. **Refresh data.** Run `npm run design:build`. This regenerates `tokens.json`, the manifest
   skeleton (merge-preserving descriptions), and the DESIGN.md token table. Do not edit these
   outputs by hand.
3. **Author prose — DESIGN.md, only between `<!-- prose:* -->` markers:**
   ...
   - `foundations`: how to use semantic color/typography/shape/spacing tokens; that dark theme
     overrides live in `tokens.json` under `themes.dark`; that agents must use semantic tokens,
     never raw `--tedi-*` primitives.
