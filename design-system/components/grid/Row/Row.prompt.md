Row from @tedi-design-system/react. Use via `window.Tedi.Row` (bundle loaded from the root `_ds_bundle.js`). Wrap the tree in `<ThemeProvider>` (full provider chain in README.md — components read theme/i18n from that context).

Variants (see `Row.html`): Default.
