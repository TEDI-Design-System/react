Pagination from @tedi-design-system/react. Use via `window.Tedi.Pagination` (bundle loaded from the root `_ds_bundle.js`). Wrap the tree in `<ThemeProvider>` (full provider chain in README.md — components read theme/i18n from that context).

Variants (see `Pagination.html`): Default, First, Last, All Properties Shown, Without Results Number, Without Dropdown, Controlled Page, Few Pages, Many Pages Ellipsis, Huge Page Count, Wider Siblings, Transparent, Top Bottom Split, Disabled Boundary Arrows, Arrows With Labels, Custom Arrow Icons, Arrows Primary Variant, Borders, Responsive Visibility, Mobile Picker, Custom Labels, Show All Page Size.
...
```jsx
/* All Properties Shown */ compose(S, "AllPropertiesShown")
```

### WithoutResultsNumber

```jsx
/* Without Results Number */ compose(S, "WithoutResultsNumber")
```

### WithoutDropdown

```jsx
/* Without Dropdown */ compose(S, "WithoutDropdown")
```

### ControlledPage

```jsx
/* Controlled Page */ compose(S, "ControlledPage")
```
...
```jsx
/* Many Pages Ellipsis */ compose(S, "ManyPagesEllipsis")
```

### HugePageCount

```jsx
/* Huge Page Count */ compose(S, "HugePageCount")
```

### WiderSiblings

```jsx
/* Wider Siblings */ compose(S, "WiderSiblings")
```
...
```jsx
/* Top Bottom Split */ compose(S, "TopBottomSplit")
```

### DisabledBoundaryArrows

```jsx
/* Disabled Boundary Arrows */ compose(S, "DisabledBoundaryArrows")
```

### ArrowsWithLabels

```jsx
/* Arrows With Labels */ compose(S, "ArrowsWithLabels")
```

### CustomArrowIcons

```jsx
/* Custom Arrow Icons */ compose(S, "CustomArrowIcons")
```

### ArrowsPrimaryVariant

```jsx
/* Arrows Primary Variant */ compose(S, "ArrowsPrimaryVariant")
```

### Borders

```jsx
/* Borders */ compose(S, "Borders")
```

### ResponsiveVisibility

```jsx
/* Responsive Visibility */ compose(S, "ResponsiveVisibility")
```

### MobilePicker

```jsx
/* Mobile Picker */ compose(S, "MobilePicker")
```

### CustomLabels

```jsx
/* Custom Labels */ compose(S, "CustomLabels")
```

### ShowAllPageSize

```jsx
/* Show All Page Size */ compose(S, "ShowAllPageSize")
```
