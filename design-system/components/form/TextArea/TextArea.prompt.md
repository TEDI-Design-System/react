TextArea from @tedi-design-system/react. Use via `window.Tedi.TextArea` (bundle loaded from the root `_ds_bundle.js`). Wrap the tree in `<ThemeProvider>` (full provider chain in README.md — components read theme/i18n from that context).

Variants (see `TextArea.html`): Default, Sizes, States, With Hint, Hint Text And Character Count, Only Character Count, Text Value, Placeholder, Default Value, Height Examples.
...
interface TextAreaProps {
  ...
  maxRows?: number;
  /** Fixed height for the textarea (e.g. '200px', '12rem', 240). Ignored when autoGrow = true. */
  height?: string | number;
  /** Maximum height of the textarea. - When `autoGrow` is enabled, this limits how tall the textarea can grow. - When `autoGrow` is disabled, this limits the fixed height. */
...
### HintTextAndCharacterCount

```jsx
/* Hint Text And Character Count */ compose(S, "HintTextAndCharacterCount")
```

### OnlyCharacterCount

```jsx
/* Only Character Count */ compose(S, "OnlyCharacterCount")
```

### TextValue

```jsx
/* Text Value */ compose(S, "TextValue")
```
...
```jsx
/* Default Value */ compose(S, "DefaultValue")
```

### HeightExamples

```jsx
/* Height Examples */ compose(S, "HeightExamples")
```
