DateTimeField from @tedi-design-system/react. Use via `window.Tedi.DateTimeField` (bundle loaded from the root `_ds_bundle.js`). Wrap the tree in `<ThemeProvider>` (full provider chain in README.md — components read theme/i18n from that context).

Variants (see `DateTimeField.html`): Default, Size, States, Field Options, Predefined Time Slots, No Times Available, Per Day Time Slots, Multi Steps, Range, Range Predefined Time Slots, Controlled, Date Constraints, Year Grid, Native.
...
```jsx
/* No Times Available */ compose(S, "NoTimesAvailable")
```

### PerDayTimeSlots

```jsx
/* Per Day Time Slots */ compose(S, "PerDayTimeSlots")
```

### MultiSteps

```jsx
/* Multi Steps */ compose(S, "MultiSteps")
```
...
### RangePredefinedTimeSlots

```jsx
/* Range Predefined Time Slots */ compose(S, "RangePredefinedTimeSlots")
```
...
```jsx
/* Date Constraints */ compose(S, "DateConstraints")
```
