DateField from @tedi-design-system/react. Use via `window.Tedi.DateField` (bundle loaded from the root `_ds_bundle.js`). Wrap the tree in `<ThemeProvider>` (full provider chain in README.md — components read theme/i18n from that context).

Variants (see `DateField.html`): Default, Size, States, Field Options, Value Type, On Click Type, Multiple Values, Multiple Tag Layout, Range, Disabled Weekends, Show Week Count, Multiple Months, Year Grid, Grid Picker First, Month Year Only, Year Only, With Footer, Available Days, Native Picker, Modal Picker, Responsive Modal Picker, Modal Pickers.
...
```jsx
/* Multiple Values */ compose(S, "MultipleValues")
```

### MultipleTagLayout

```jsx
/* Multiple Tag Layout */ compose(S, "MultipleTagLayout")
```

### Range

```jsx
/* Range */ compose(S, "Range")
```

### DisabledWeekends

```jsx
/* Disabled Weekends */ compose(S, "DisabledWeekends")
```

### ShowWeekCount

```jsx
/* Show Week Count */ compose(S, "ShowWeekCount")
```

### MultipleMonths

```jsx
/* Multiple Months */ compose(S, "MultipleMonths")
```

### YearGrid

```jsx
/* Year Grid */ compose(S, "YearGrid")
```

### GridPickerFirst

```jsx
/* Grid Picker First */ compose(S, "GridPickerFirst")
```

### MonthYearOnly

```jsx
/* Month Year Only */ compose(S, "MonthYearOnly")
```

### YearOnly

```jsx
/* Year Only */ compose(S, "YearOnly")
```
...
```jsx
/* Available Days */ compose(S, "AvailableDays")
```
...
```jsx
/* Modal Pickers */ compose(S, "ModalPickers")
```
