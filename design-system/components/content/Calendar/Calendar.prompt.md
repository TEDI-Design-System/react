Calendar from @tedi-design-system/react. Use via `window.Tedi.Calendar` (bundle loaded from the root `_ds_bundle.js`). Wrap the tree in `<ThemeProvider>` (full provider chain in README.md — components read theme/i18n from that context).

Variants (see `Calendar.html`): Default, With Footer, Multiple Selected Dates, With Legend, Availability, Range, No Controls, With Weeks Count, Month View, Year View.
...
```jsx
/* Multiple Selected Dates */ compose(S, "MultipleSelectedDates")
```

### WithLegend

```jsx
/* With Legend */ compose(S, "WithLegend")
```

### Availability

```jsx
/* Availability */ compose(S, "Availability")
```
...
```jsx
/* No Controls */ compose(S, "NoControls")
```

### WithWeeksCount

```jsx
/* With Weeks Count */ compose(S, "WithWeeksCount")
```

### MonthView

```jsx
/* Month View */ compose(S, "MonthView")
```

### YearView

```jsx
/* Year View */ compose(S, "YearView")
```
