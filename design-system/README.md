...
# Tedi (@tedi-design-system/react@0.0.0-semantic-version)

This design system is the published @tedi-design-system/react React library, bundled as a single
browser global. All 82 components are the real upstream code.

## Where things are

- `_ds_bundle.js` — the whole-DS bundle at the project root; loads every component to `window.Tedi`. First line is a `/* @ds-bundle: … */` metadata header.
...
Components are then available at `window.Tedi.*`. Mount into a dedicated child node (e.g. `<div id="ds-root">`), not the host page's own React root, so the two trees don't collide:

```jsx
const { Accordion } = window.Tedi;
...
```jsx
<ThemeProvider theme={"default"}><PrintingProvider><LabelProvider locale={"et"}>{children}</LabelProvider></PrintingProvider></ThemeProvider>
```

## Tokens

1330 CSS custom properties from @tedi-design-system/core. Names are
preserved verbatim from upstream. See `tokens/` for the full list.

- **color** (270): `--accordion-action-color`, `--alert-default-border-danger`, `--alert-default-border-info`, …
- **spacing** (147): `--alert-default-padding-x`, `--alert-default-padding-x-sm`, `--alert-default-padding-y`, …
- **typography** (51): `--tedi-line-height-00`, `--tedi-line-height-01`, `--tedi-line-height-02`, …
- **radius** (33): `--tedi-radius-00`, `--tedi-radius-01`, `--tedi-radius-02-default`, …
- **other** (829): `--tedi-accent-100`, `--tedi-accent-200`, `--tedi-accent-300`, …

## Components

### content
- `Accordion` (compound: `Accordion.Item`)
- `Calendar`
- `Card` (compound: `Card.Content`, `Card.Header`, `Card.Notification`)
- `Carousel` (compound: `Carousel.Header`, `Carousel.Content`, `Carousel.Footer`, `Carousel.Navigation`, `Carousel.Indicators`)
- `Collapse`
- `EmptyState`
- `HeadingWithIcon`
- `Label`
- `List` (compound: `List.Item`)
- `Section`
- `Table` (compound: `Table.Toolbar`, `Table.ColumnsMenu`, `Table.HeaderButton`)
- `TextGroup` (compound: `TextGroup.List`)
- `Truncate`
...
- `Ellipsis`
- `HashTrigger`
- `OptionContent` (compound: `OptionContent.Label`, `OptionContent.Meta`)
- `Print`
- `ScrollFade`
- `ScrollVisibility`
- `Separator`
...
### navigation
- `Breadcrumbs`
- `HorizontalStepper` (compound: `HorizontalStepper.Item`)
- `Link`
- `Pagination`
- `TableOfContents` (compound: `TableOfContents.Item`, `TableOfContents.Collapsible`)
...
### choicegroup
- `Checkbox`
- `ChoiceGroup` (compound: `ChoiceGroup.Item`)
- `Radio`
...
- `FileDropzone`
- `FileUpload`
- `InputGroup` (compound: `InputGroup.Prefix`, `InputGroup.Suffix`, `InputGroup.Input`)
- `NumberField`
- `Search`
- `Select`
- `Slider`
...
### overlay
- `Dropdown` (compound: `Dropdown.Trigger`, `Dropdown.Content`, `Dropdown.Item`, `Dropdown.Separator`)
- `InfoTooltip` — An info button that reveals a tooltip on hover/focus  the standard
- `Modal` (compound: `Modal.Trigger`, `Modal.Content`, `Modal.Header`, `Modal.Body`, `Modal.Footer`, `Modal.Closer`)
- `Popover` (compound: `Popover.Trigger`, `Popover.Content`)
...
### layout
- `Footer` (compound: `Footer.Side`, `Footer.Body`, `Footer.Section`, `Footer.Bottom`)
- `Header` (compound: `Header.Logo`, `Header.Center`, `Header.Actions`, `Header.Language`, `Header.Login`, `Header.Logout`, …)
- `HideAt`
- `ShowAt`
- `SideNav` (compound: `SideNav.Toggle`, `SideNav.Item`, `SideNav.Dropdown`, `SideNav.Mobile`)
- `TopNav` (compound: `TopNav.Item`, `TopNav.Group`, `TopNav.SubItem`, `TopNav.Separator`)
- `VerticalSpacing` (compound: `VerticalSpacing.Item`)
...
### loader
- `ProgressBar`
- `Skeleton` (compound: `Skeleton.Block`)
- `Spinner`
