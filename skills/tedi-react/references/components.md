...
### TableOfContents
Navigational TOC for long pages. **Compound API** — composed from `TableOfContents.Item` children; nest items by placing `Item`s inside an `Item` (alongside its link). Renders a (optionally sticky) card. Mark the current section with `activeId` for the left accent bar + active link colour. Nested items are visible by default; set `collapseInactive` to make the list an accordion (only the active branch stays expanded). For small viewports there's a separate `TableOfContents.Collapsible` (bottom bar + bottom-sheet overlay) — render it instead of the card on mobile (e.g. via `ShowAt`/`HideAt`).

- `<TableOfContents>` props: `heading?: string | null` (default LabelProvider `table-of-contents.title`; pass `null` for a **headless** list — no visible heading, `nav` still gets the localised `aria-label`), `variant?: 'default' | 'transparent' = 'default'` (`transparent` drops the card border/background and shows a continuous grey left rail — the active item's segment turns blue), `padding?: number` (inner container padding in rem; defaults to the card's medium padding token), `headingLevel?: 'h1'..'h6' = 'h3'` (configurable semantic heading level — visual style stays H4; set it to fit the page's heading outline and avoid skipped heading levels), `ariaLabel?: string` (override the nav landmark's accessible name; when empty it falls back to the heading via `aria-labelledby`, or the localised title when headless — use it to disambiguate multiple TOCs on a page), `activeId?`, `collapseInactive?: boolean = false` (by default every item's nested children stay visible; set it to collapse branches off the active trail, so only the branch leading to `activeId` is expanded — accordion behaviour), `numbered?` (ordered list with auto hierarchical numbers `1.` / `2.` / `2.1`), `bordered?: boolean = false` (draws a divider between items and a border under the last one), `sticky?: boolean = true`, `className?`. For a trailing entry (e.g. a summary or "back to top" link) set `separator` on the item above it — there is no `footer` slot. Children must be `TableOfContents.Item` elements (direct children — don't wrap them in another component). Item rows carry vertical padding via the `--table-of-contents-padding-y` token; the selected item's left accent bar spans the full row height.
- **Breakpoint support:** the visual props — `heading`, `headingLevel`, `ariaLabel`, `variant`, `padding`, `bordered`, `collapseInactive`, `numbered`, `sticky`, `className` — accept per-breakpoint overrides via the `sm`/`md`/`lg`/`xl`/`xxl` keys (mobile-first: a bare value is the `xs` baseline; `defaultServerBreakpoint?` sets the SSR baseline). e.g. `variant="transparent" md={{ variant: 'default' }}`, or drop `sticky` on mobile. `children` and `activeId` are not breakpoint-aware.
- `<TableOfContents.Collapsible heading? ariaLabel? activeId? numbered? sticky? hideOnScroll? className>` — mobile variant. No `headingLevel` (the sheet shows the title as a label, not a semantic heading; `ariaLabel` — or the `heading` / localised title — names the sheet's `<nav>`). A bar (`heading` + an "open" link) that reveals the same list in a bottom-sheet `Modal` (dimmed backdrop, Escape / backdrop to dismiss). Takes the same `TableOfContents.Item` children. `sticky?: boolean = true` pins the bar to the bottom of the viewport (`position: fixed`); set `false` to render it inline in normal flow. `hideOnScroll?: boolean = false` — when the bar is `sticky`, slide it out of view while the page scrolls **down** and bring it back on scroll **up** (no effect when `sticky={false}`; respects `prefers-reduced-motion`). No `variant`/`padding` (it's not a card). Show it only on small screens; keep the desktop `<TableOfContents>` card for wider ones.
- `<TableOfContents.Item id? separator? slot?>` — its non-`Item` children are the link / label; nested `TableOfContents.Item` children become its sub-items. `separator` draws a divider below just that item (vs `bordered` on the parent, which divides every row). `slot?: ReactNode` renders trailing content at the end of the row (right-aligned — e.g. a count `Tag`), kept out of the link's accessible name.
...
- When `sticky` (default), the card is capped to the visible viewport region (`max-height: calc(100dvh - <sticky top offset> - <bottom gap>)`, where the top offset accounts for a sticky layout header via the `Affix` offsets) and scrolls internally, so every item stays reachable when it's taller than the viewport (e.g. narrow widths at high zoom — WCAG 1.4.10 Reflow).
