...
import ColorAndIconPicker from './color-and-icon-picker';
...
const meta: Meta<typeof ColorAndIconPicker> = {
  component: ColorAndIconPicker,
  title: 'Community/Map components/ColorAndIconPicker',
};

export default meta;
type Story = StoryObj<typeof ColorAndIconPicker>;

const InteractiveTemplate = (args: React.ComponentProps<typeof ColorAndIconPicker>) => {
  const [selectedValue, setSelectedValue] = useState(args.options[0].value);

  return (
    <>
      <ColorAndIconPicker {...args} selectedValue={selectedValue} onSelect={setSelectedValue} />
    </>
  );
};
