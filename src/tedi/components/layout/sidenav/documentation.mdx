...
>
> Three shell mistakes to avoid: giving the nav column its own padding (it must sit flush
> against the shell edge), letting the nav column size to its content (it has to stretch to
> the footer), and making the column `sticky` instead of the inner rail.
