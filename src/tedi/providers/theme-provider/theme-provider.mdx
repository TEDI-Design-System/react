...
You can interactively switch themes in Storybook using the toolbar at the top right (🖌️ **Theme** selector).
...
This setup ensures the `<html>` class and the ThemeProvider both start with the same value — so your UI loads with the correct theme immediately.
