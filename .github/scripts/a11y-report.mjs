...
const storyLink = (component, story) => {
  ...
  return `[${story}](${storybookBase}/?path=/story/${sanitize(component)}--${sanitize(story)}&addonPanel=storybook/a11y/panel)`;
};

// --- diff scoping: map story title -> source dir, flag changed components ---
let changedFiles = null;
if (changedPath && fs.existsSync(changedPath)) {
  changedFiles = fs.readFileSync(changedPath, "utf8").split("\n").map((s) => s.trim()).filter(Boolean);
}
...
const readTodos = () => {
  ...
  for (const raw of lines) {
    ...
    if (t) { last = { component: t[1].trim(), story: t[2].trim() }; continue; }
...
const compTable = (map) => {
  ...
  for (const [component, { rules, stories }] of [...map.entries()].sort()) {
    const r = [...rules.entries()].sort((a, b) => b[1] - a[1]).map(([k, c]) => `\`${k}\`${c > 1 ? ` ×${c}` : ""}`).join(", ");
...
if (blockingStories === 0) {
  ...
  out.push("");
  out.push(scoped ? "No accessibility violations in the components changed by this PR." : "All TEDI-Ready stories pass automated axe checks.");
} else {
  out.push(`## ♿ Accessibility — ❌ ${blockingStories} blocking violation${blockingStories === 1 ? "" : "s"} in changed component${blocking.size === 1 ? "" : "s"}`);
  out.push("");
  out.push("These are in components this PR changed and must be fixed (or marked `parameters: { a11y: { test: 'todo' } }` with justification).");
  out.push("");
